...
function requestJson(command=null)
{
	...
	var req = null;
	var useWs = (ws && ws.readyState === StubWebSocket.OPEN);
	...
	.then(json => {
		...
		if (!pJson || isEmpty(pJson)) setTimeout(()=>{
			loadPresets(()=>{
				wsRpt = 0;
				if (!(ws && ws.readyState === StubWebSocket.OPEN)) makeWS();
			});
		},25);
		reqsLegal = true;
		retry = false;
...
function toggleLiveview()
{
	...
	isLv = !isLv;
	let wsOn = ws && ws.readyState === StubWebSocket.OPEN;
